...
class SimpleButton():
    ...
    def __intit__(self, window, loc, up, down):
        self.window = window
        self.loc = loc
    ...
    def handleEvent(self, eventObj):
        ...
        if self.state == SimpleButton.STATE_IDLE:
            if (eventObj.type == MOUSEBUTTONDOWN) and eventPointInButtonRect:
                self.state = SimbleButton.STATE_ARMED
        ...
        elif self.state == SimpleButton.STATE_DISARMED and eventPointInButtonRect:
            if eventPointInButtonRect:
                self.state = SimpleButtone.STATE_ARMED
